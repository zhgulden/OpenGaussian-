...
def prepare_output_and_logger(args):    
    if not args.model_path:
        ...
        args.model_path = os.path.join("./output/M2/", unique_str[0:10])
...
def construct_pseudo_ins_feat(scene : Scene, renderFunc, renderArgs, 
    ...
    if mode == "lang":
        ...
        visibility_info = torch.zeros(root_num * leaf_num, len(sorted_train_cameras)).cuda()  # [k1*k2, num_imgs]
        ...
        for root_id, _ in enumerate(tqdm(range(root_num), desc="mapping")):
            ...
            for v_id, view in enumerate(sorted_train_cameras):
                ...
                if len(occured_leaf_id) > 0:
                    ...
                    visibility_scores = rendered_leaf_cluster_silhouettes.sum(dim=[1, 2])
                    visibility_info[occured_leaf_id, v_id] = visibility_scores
                else:
                    if view.data_on_gpu and save_memory:
                        view.to_cpu()
        ...
        leaf_per_view_is_matched = match_info[:, :, 2].bool() # [k1*k2, num_cam] boolean for match
        ...
        # Multi-View CLIP Feature Fusion:
        # Aggregate and average CLIP features from all views where a leaf cluster is visible and matched.
        per_leaf_feat_sum = torch.zeros(root_num * leaf_num, 512).cuda()  # [k1*k2, 512] 
        per_leaf_weight_sum = torch.zeros(root_num * leaf_num).cuda()
        for v_id, view in enumerate(sorted_train_cameras):
            if not view.data_on_gpu:
                view.to_gpu()
            if sam_level == 0:
            ...
            # Get features for matched masks in the current view
            current_view_mask_indices = leaf_per_view_matched_mask[:, v_id]
            single_view_leaf_feat = curr_view_lang_feat[current_view_mask_indices]
            
            # Use only features from matched clusters, others will be zero.
            is_matched_in_view = leaf_per_view_is_matched[:, v_id]

            # Get match scores and visibility scores to compute combined weight
            match_scores = match_info[:, v_id, 1]
            visibility_scores = visibility_info[:, v_id]
            combined_weights = match_scores * visibility_scores
            
            # Accumulate features from matched views
            per_leaf_feat_sum += single_view_leaf_feat * (combined_weights * is_matched_in_view).unsqueeze(1)
            per_leaf_weight_sum += combined_weights * is_matched_in_view

            if view.data_on_gpu and save_memory:
                view.to_cpu()

        # average language features [k1*k2, 512] 
        per_leaf_feat = per_leaf_feat_sum / (per_leaf_weight_sum + 1e-6).unsqueeze(1)
